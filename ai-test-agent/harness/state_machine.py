...
_TRANSITIONS: dict[State, set[State]] = {
    ...
    State.SETUP: {State.UI_TEST, State.API_TEST, State.WHITEBOX_TEST, State.FAILED},
    # 多 Agent 串行：每个子 Agent 一个 TEST 状态，允许顺序推进后统一进入 VERIFY
    State.UI_TEST: {State.API_TEST, State.WHITEBOX_TEST, State.VERIFY, State.FAILED},
    State.API_TEST: {State.UI_TEST, State.WHITEBOX_TEST, State.VERIFY, State.FAILED},
    State.WHITEBOX_TEST: {State.VERIFY, State.FAILED},
