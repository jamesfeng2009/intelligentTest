...
class State(Enum):
    ...
    SECURITY_TEST = "SECURITY_TEST"        # 安全测试用例生成 + 静态审查（自定义 Agent 示例）
    VERIFY = "VERIFY"
    REPORT = "REPORT"
    DONE = "DONE"
    FAILED = "FAILED"
...
_TRANSITIONS: dict[State, set[State]] = {
    ...
    State.SETUP: {State.UI_TEST, State.API_TEST, State.WHITEBOX_TEST, State.FUNCTIONAL_TEST, State.SECURITY_TEST, State.FAILED},
    # 多 Agent 串行：每个子 Agent 一个 TEST 状态，允许顺序推进后统一进入 VERIFY
    State.UI_TEST: {State.API_TEST, State.WHITEBOX_TEST, State.FUNCTIONAL_TEST, State.SECURITY_TEST, State.VERIFY, State.FAILED},
    State.API_TEST: {State.UI_TEST, State.WHITEBOX_TEST, State.FUNCTIONAL_TEST, State.SECURITY_TEST, State.VERIFY, State.FAILED},
    State.WHITEBOX_TEST: {State.FUNCTIONAL_TEST, State.SECURITY_TEST, State.VERIFY, State.FAILED},
    # 功能用例生成 → 独立评审（B 模型）；评审后进入其它测试或 VERIFY
    State.FUNCTIONAL_TEST: {State.CASE_REVIEW, State.SECURITY_TEST, State.VERIFY, State.FAILED},
    State.CASE_REVIEW: {State.SECURITY_TEST, State.VERIFY, State.FAILED},
    # 安全测试（自定义 Agent 接入示例）：生成 + 静态审查后统一进 VERIFY
    State.SECURITY_TEST: {State.VERIFY, State.FAILED},
